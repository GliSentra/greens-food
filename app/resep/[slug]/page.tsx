...
import { recipesData, getRecipeBySlug, Recipe } from '@/app/data/recipes';
...
export default function RecipeDetailPage({ params }: { params: { slug: string } }) {
    ...
    useEffect(() => {
        getRecipeBySlug(params.slug).then(data => {
            if (!data) notFound();
            setRecipe(data);
        });
    }, [params.slug]);
    ...
    const relatedRecipes = recipesData.filter(r => r.slug !== recipe.slug).slice(0, 3);
