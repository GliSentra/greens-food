...
export default function ProductView({ product }: { product: Product }) {
    ...
    const shareText = encodeURIComponent(`Lihat produk microgreens menarik ini: ${product.name}`);
            ...
            <div className="container mx-auto px-6 pb-12 sm:pb-16">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-3">
                                {product.variants.map(variant => (
                                    <button key={variant.size} onClick={() => setSelectedSize(variant.size)}
                                        className={`px-5 py-2 rounded-lg border-2 font-semibold transition-colors duration-200
                               ${selectedSize === variant.size ? 'bg-green-600 text-white border-green-600' : 'bg-white text-gray-700 border-gray-300 hover:border-green-500'}`}
                                    >
                                        {variant.size}
                                    </button>
                                ))}
                        ...
                        <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="w-full mt-2 bg-green-600 text-white font-bold py-4 px-6 rounded-lg hover:bg-green-700 transition duration-300 flex items-center justify-center text-base sm:text-lg">
